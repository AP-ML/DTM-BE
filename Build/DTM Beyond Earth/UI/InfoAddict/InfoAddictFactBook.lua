...
local EmpirePage = 1;
local EmpirePageMax = 1; --It's been merged all into 1 page now. v4 edit.
...
-- Returns a table of ranking types and their associated text.
...
function RankingTitles()
  ...
  rankings[6] = Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RANKTITLE_AFFINITIES");
...
function RankingUnits()
  ...
  units[6] = Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RANKUNITS_AFFINITIES");
...
function GetDemoValue(pid, rankType)
  ...
  elseif (rankType == 3) then
  	-- CivBE Still has CalculateGrossGold 
  ...
  elseif (rankType == 6) then
    return computeAffinityLevelSum(pid);
  ...
  elseif (rankType == 14) then
  	return p:GetTotalCulturePerTurn();
...
function BuildEmpireStats()
  ...
  --[[ no longer needed with 1 page. V4 edit.
  ...
  --if (EmpirePage == 1) then -- no longer need, only 1 page. V4 edit.
  ...
  BuildEmpireStatRow(pid, 12, numcities, Locale.ConvertTextKey("TXT_KEY_INFOADDICT_EMPIREATTR_NUMCITIES"));
	...
	local affinity = FormatDemoValue(GetDemoValue(pid, 17), "#,###,###,###") ;
	BuildEmpireStatRow(pid, 17, affinity, Locale.ConvertTextKey("TXT_KEY_INFOADDICT_EMPIREATTR_AFFINITY"));
...
--[[ -- No longer needed, as everything is moved into one page. V4 edit.
