...
-- keyType.defensive = 1;
keyType.denounce = 2;
keyType.DoF = 3; -- Now Cooperation Agreement.
keyType.borders = 4;
keyType.research = 5;
keyType.GPT = 6; -- Now Energy per Turn.
keyType.resource = 7;
keyType.hostile = 8;
keyType.guarded = 9;
keyType.afraid = 10;
...
function politicalView()
  ...
  tooltipData["DoF"] = {}; -- cooperation agreement.
  tooltipData["borders"] = {};
      ...
      for targetPid = thisPid + 1, GameDefines.MAX_MAJOR_CIVS-1, 1 do
        if( hasMetCiv(targetPid) and Players[targetPid]:IsAlive()) then
          ...
          if (thisTeam:IsAtWar(targetTid) and isKeySelected(keyType.war)) then
            ...
            if (thisPlayer:IsCoopAgreement(targetPid) and isKeySelected(keyType.DoF) ) then
  ...
  for thisPid = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
    if (tooltipData["war"][thisPid] ~= nil) then
      ...
      -- Defensive pacts - No longer needed for BE
      ...
      -- Declaration of Friendship -- now Cooperation Agreement.
        ...
        repeat
          ...
          if (toPid ~= fromPid) then
            ...
            if (hasMetCiv(fromPid) and Players[fromPid]:IsAlive() and 
              ...
              if( TradeableItems.TRADE_ITEM_ENERGY_PER_TURN == itemType ) then
                
                local goldamount = data1;
                local exportstr = tooltipPad .. "[ICON_ENERGY]" .. iconPad .. goldamount .. " " .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_GPT_TO") ..
                                  " " .. toName .. " (" .. turnsLeft .. ")";
                local importstr = tooltipPad .. "[ICON_ENERGY]" .. iconPad .. goldamount .. " " .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_GPT_FROM") ..
                                  " " .. fromName .. " (" .. turnsLeft .. ")";
...
function showConnector(civ1, civ2, color)
  ...
  if (not isCivSelected(civ1) and not isCivSelected(civ2)) then
	logger:debug("showConnector: caught in return if statement!");
    return false;
  end;
