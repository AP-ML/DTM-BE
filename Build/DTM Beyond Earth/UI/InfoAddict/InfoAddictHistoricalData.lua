...
function DrawGraph(graphType)
  ...
  for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
      ...
      if ( doHighlight == true ) then
        ...
        if (pidHighlights[pid] == true) then
          ...
          alpha = .3;
			...
			local _, primaryColor = pPlayer:GetPlayerColors();
			local color = Vector4(primaryColor.x, primaryColor.y, primaryColor.z, alpha);
      ...
      local previousX = nil;
        ...
        -- specific pixel. Occasionally, we'll get a gap and this fills that in.
        ...
        -- Draw the horizontal line if we have a non-nil value and we're not in smooth mode.
  ...
  for i = 0, numLabels - 1, 1 do
    ...
    local turnText = thisTurn;     --getYearString(thisTurn);
    ...
    if (iPoint == pointsPerPlayer) then
      ...
      turnText = thisTurn;      --getYearString(thisTurn);
    end
...
function HighlightSelected(type)
  ...
  if (type == "affinity") then
	Controls.AffinitySelectHighlight:SetHide(false);
  else
	Controls.AffinitySelectHighlight:SetHide(true);
  end;
...
function OnAffinity()
 DrawGraph("affinity");
 HighlightSelected("affinity");
 end
 Controls.AffinityButton:RegisterCallback( Mouse.eLClick, OnAffinity);


-- Controls which graph type bar to view
