...
local g_NotificiationSummaries = {};
g_NotificiationSummaries[-1] = -1;
...
function OnEndTurnDirty()
	...
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_PRODUCTION) then
		...
		local blockingNotificationIndex = player:GetEndTurnBlockingNotificationIndex();
		if g_NotificiationSummaries[blockingNotificationIndex] ~= nil then
			strEndTurnMessage = g_NotificiationSummaries[blockingNotificationIndex];
			strButtonToolTip = strEndTurnMessage;
		else
...
function OnNotificationAdded( Id, type, toolTip, strSummary, iGameValue, iExtraGameData, ePlayer )
	--print ( "NOTIFICATION ADDED " .. tostring(Id) .. "  " .. type .. "  " .. strSummary .. "  " .. tostring(iGameValue) .. "  " .. tostring(iExtraGameData) .. "  " .. tostring(ePlayer));
	if type == NotificationTypes.NOTIFICATION_PRODUCTION then
		local sTextKey = "";
		
		-- Get text key for name of finished product
		if iGameValue == 0 then -- unit
			sTextKey = GameInfo.Units[iExtraGameData].Description;
		elseif iGameValue == 1 then -- building
			sTextKey = GameInfo.Buildings[iExtraGameData].Description;
		end
		
		-- Convert text key to usable string
		if sTextKey ~= "" then
			local sName = Locale.ConvertTextKey("TXT_KEY_RESEARCH_FINISHED") .. " " .. Locale.ConvertTextKey(sTextKey);
			sName = Locale.ToUpper(sName);
			strSummary = sName;
			
			-- Add to table
			g_NotificiationSummaries[Id] = strSummary;
			
			-- And refresh if required
			if Players[Game.GetActivePlayer()]:GetEndTurnBlockingNotificationIndex() == Id then
				OnEndTurnDirty();
			end
		end
	end
