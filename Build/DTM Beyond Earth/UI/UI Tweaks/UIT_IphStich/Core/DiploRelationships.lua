...
--[[ WAR
		...
		controlTable.FastTurn:RegisterCheckHandler( function (bIsChecked) OnFastTurnChecked (bIsChecked, iPlayerLoop); end );
		controlTable.FastTurn:SetCheck(m_isFastTurnForPlayer[iPlayerLoop] == true);
...
function OnFastTurnChecked ( bIsChecked, playerID )
	
	m_isFastTurnForPlayer[playerID] = bIsChecked;
...
Controls.FastAliens:RegisterCheckHandler( function (bIsChecked) OnFastTurnChecked (bIsChecked, -1); end );

function WarStateChangedHandler (iTeam1, iTeam2, bWar)
	if bWar and iTeam1 == Game.GetActiveTeam() then
		for i,j in pairs(m_isFastTurnForPlayer) do
			if j and Players[i]:GetTeam() == iTeam2 then
				m_isFastTurnForPlayer[i] = false;
				UpdateDisplay();
			end
		end
	end
...
function OnTurnStart (playerID)
	...
	if (player:IsAlien()) then playerID = -1; end;
	
	local fastTurn = m_isFastTurnForPlayer[playerID];
	
	if (fastTurn == nil) then fastTurn = false; end;
	
	local quickCombat = false;
	local quickMovement = false;
	
	if (PreGame.IsMultiplayerGame() or PreGame.IsHotSeatGame()) then
		quickCombat = fastTurn or OptionsManager.GetMultiplayerQuickCombatEnabled_Cached();
		quickMovement = fastTurn or OptionsManager.GetMultiplayerQuickMovementEnabled_Cached();
	else
		quickCombat = fastTurn or OptionsManager.GetSinglePlayerQuickCombatEnabled_Cached();
		quickMovement = fastTurn or OptionsManager.GetSinglePlayerQuickMovementEnabled_Cached();
	end
	
	local options = {};
	table.insert(options, { "GAMEOPTION_QUICK_COMBAT", quickCombat });
	table.insert(options, { "GAMEOPTION_QUICK_MOVEMENT", quickMovement });
	Network.SendGameOptions(options);
end
--Events.NewGameTurn.Add( OnTurnStart );
--Events.ActivePlayerTurnStart.Add( OnTurnStart );
Events.RemotePlayerTurnStart.Add( OnTurnStart );
Events.AIProcessingStartedForPlayer.Add( OnTurnStart );
