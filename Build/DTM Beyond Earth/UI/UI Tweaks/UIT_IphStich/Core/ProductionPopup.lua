...
function ProductionSelected( ePurchaseEnum, iData)
    ...
    if g_IsProductionMode then
		...
		-- Handle queue, under special circumstances
		...
		if qLength > 0 then
			...
			local queuedSave;
			local queuedRush;
			queuedOrderType, queuedData1, queuedData2, queuedSave, queuedRush = city:GetOrderFromQueue( qLength-1 );
			
			-- if previous item is a maintain item
			if queuedOrderType == OrderTypes.ORDER_MAINTAIN then
				-- if new item is maintain item
				-- for some reason I can't swap the new item with the currently-in-production item, so instead we delete it if it is a maintain item
				if ePurchaseEnum == g_MAINTAIN_PROCESS or qLength == 1 then
					-- delete prev item
					Game.SelectedCitiesGameNetMessage(GameMessageTypes.GAMEMESSAGE_POP_ORDER, qLength-1);
					LuaEvents.CityQueueDirty( g_isProductionQueueOpen );
				else
					-- swap items
					Game.SelectedCitiesGameNetMessage(GameMessageTypes.GAMEMESSAGE_SWAP_ORDER, qLength-1);
				end
			end
