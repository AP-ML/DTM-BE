...
---print("MORE FILTERS MOD DETECTED")
...
---x my mod variables
local g_filterTable2			= {};					-- Contains table of filters
local g_filterTable3			= {};					-- Contains table of filters
local g_filterTable4			= {};					-- Contains table of filters
local g_currentFilterPulldownButton	= nil;					-- Tracks Last clicked filter button/pulldown/column for OnFilterClicked()
...
function InitialSetup()
	...
	-- Setup filter area
	--- centers the text (fixes the anchor) for the text on the button (Controls.FilterPulldown:GetButton():GetTextControl(), so text control of the button of the pulldown)
	...
	---x my filter label anchor fix
	---center the text on the pulldown button, should do this for each filter area
	local pullDownButton2 = Controls.FilterPulldown2:GetButton();	
	pullDownButton2:SetText( "   "..Locale.ConvertTextKey("TXT_KEY_TECHWEB_FILTER"));	
	local pullDownLabel2 = pullDownButton2:GetTextControl();
	pullDownLabel2:SetAnchor("L,C");
	pullDownLabel2:ReprocessAnchoring();

	---center the text on the pulldown button, should do this for each filter area
	local pullDownButton3 = Controls.FilterPulldown3:GetButton();	
	pullDownButton3:SetText( "   "..Locale.ConvertTextKey("TXT_KEY_TECHWEB_FILTER"));	
	local pullDownLabel3 = pullDownButton3:GetTextControl();
	pullDownLabel3:SetAnchor("L,C");
	pullDownLabel3:ReprocessAnchoring();

	---center the text on the pulldown button, should do this for each filter area
	local pullDownButton4 = Controls.FilterPulldown4:GetButton();	
	pullDownButton4:SetText( "   "..Locale.ConvertTextKey("TXT_KEY_TECHWEB_FILTER"));	
	local pullDownLabel4 = pullDownButton4:GetTextControl();
	pullDownLabel4:SetAnchor("L,C");
	pullDownLabel4:ReprocessAnchoring();
	---x end filter label anchor fix

	-- Hard coded/special filters:
	table.insert( g_filterTable, { "", "TXT_KEY_TECHWEB_FILTER_NONE", nil } );
	---x insert the "no filter selected" entry at the start of each filter pulldown
	table.insert( g_filterTable2, { "", "TXT_KEY_TECHWEB_FILTER_NONE", nil } );
	table.insert( g_filterTable3, { "", "TXT_KEY_TECHWEB_FILTER_NONE", nil } );
	table.insert( g_filterTable4, { "", "TXT_KEY_TECHWEB_FILTER_NONE", nil } );

	-- Load entried into filter table from TechFilters XML data
	for row in GameInfo.TechFilters() do
		---x if else conditionals added by me to sort tech filters into their correct filter slots
		---i dont think you can add new xml attributes, so I had to use this row.ID sorting method. 
		---this means that the xml order for new filters (in TechFilters.xml) is important in determining which pulldown they end up in 
		if row.ID <= 13 then
			table.insert(g_filterTable, { row.IconString, row.Description, g_TechFilters[row.Type] });
		elseif row.ID <= 25 then
			table.insert(g_filterTable2, { row.IconString, row.Description, g_TechFilters[row.Type] });
		elseif row.ID <= 31 then
			table.insert(g_filterTable3, { row.IconString, row.Description, g_TechFilters[row.Type] });
		else---if row.ID <= 43 then     ---ignore that to the left, all filters with higher ID than 31 will end up in filter pulldown 4 as is
			table.insert(g_filterTable4, { row.IconString, row.Description, g_TechFilters[row.Type] });
		---else
			---print("Error adding TechFilter to filterTable. Filter tables may be full");
		end
	end

	--- Add all the filters taken from xml and stored in g_filterTable's to the actual button controls/dropdown
	--- I don't appear to have to do anything except copy this for each filter dropdown and change references to correct # filterPullDown or table 
	for i,filterType in ipairs(g_filterTable) do
		...
		controlTable.Button:RegisterCallback( Mouse.eLClick,  function() OnFilterClicked( filterLabel, filterFunction, Controls.FilterPulldown:GetButton() ); end );
	end
	Controls.FilterPulldown:CalculateInternals();

	---x MY MOD, see comment above the chunk above
	for i,filterType in ipairs(g_filterTable2) do
		...
		local controlTable	 = {};        
		Controls.FilterPulldown2:BuildEntry( "FilterItemInstance", controlTable );
		...
		-- Use a lambda function to organize how paramters will be passed...												
		controlTable.Button:RegisterCallback( Mouse.eLClick,  function() OnFilterClicked( filterLabel, filterFunction, Controls.FilterPulldown2:GetButton() ); end );
	end
	Controls.FilterPulldown2:CalculateInternals();

	for i,filterType in ipairs(g_filterTable3) do
		...
		local controlTable	 = {};        
		Controls.FilterPulldown3:BuildEntry( "FilterItemInstance", controlTable );
		...
		-- Use a lambda function to organize how paramters will be passed...												
		controlTable.Button:RegisterCallback( Mouse.eLClick,  function() OnFilterClicked( filterLabel, filterFunction, Controls.FilterPulldown3:GetButton() ); end );
	end
	Controls.FilterPulldown3:CalculateInternals();

	for i,filterType in ipairs(g_filterTable4) do
		...
		local controlTable	 = {};        
		Controls.FilterPulldown4:BuildEntry( "FilterItemInstance", controlTable );
		...
		-- Use a lambda function to organize how paramters will be passed...												
		controlTable.Button:RegisterCallback( Mouse.eLClick,  function() OnFilterClicked( filterLabel, filterFunction, Controls.FilterPulldown4:GetButton() ); end );
	end
	Controls.FilterPulldown4:CalculateInternals();
	---x END MY MOD
...
-- ===========================================================================
---x I added the 3 extra pullDownButton vars and their text being set to default beforehand
function RefreshFilterDisplay()
	...
	---x set all to their default labels first
	pullDownButton:SetText( "  FILTERS");--..Locale.ConvertTextKey("TXT_KEY_TECHWEB_FILTER"));
	pullDownButton2:SetText( "  BASIC RESOURCES");--..Locale.ConvertTextKey("TXT_KEY_TECHWEB_FILTER2"));
	pullDownButton3:SetText( "  STRATEGIC RESOURCES");--..Locale.ConvertTextKey("TXT_KEY_TECHWEB_FILTER3"));
	pullDownButton4:SetText( "  TILE IMPROVEMENTS");--..Locale.ConvertTextKey("TXT_KEY_TECHWEB_FILTER4"));
	---x end my default label setting

	---x then set the filter label for the current clicked pulldown
	if ( g_currentFilter ~= nil ) then
		---x reworked some stuff inside here to fix the logic for multiple filter pulldowns where only one is selectable at a time
		g_currentFilterPulldownButton:SetText( "  "..Locale.ConvertTextKey(g_currentFilterLabel));
	end
end
...
-- ===========================================================================
	---x	I added the last param "filterPulldownButton" to track which of the 4 filter dropdowns was clicked
	--- so that I could reset all non-clicked pulldowns to default text in RefreshFilterDisplay() (4 dropdowns that act as 1 large one, 1 selection at a time)
function OnFilterClicked( filterLabel, filterFunc, filterPulldownButton )		
	g_currentFilter=filterFunc;
	g_currentFilterLabel=filterLabel;
	g_currentFilterPulldownButton = filterPulldownButton;
