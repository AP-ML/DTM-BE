...
local g_firstPrevRoute;
local g_prevTradeRoutes = {};
local autoRenewRoutes = {};
...
--DM
    ...
    do
		...
		if action.Type == "SPECIALIST_SCIENTIST" then
			bShowActionButton = false;
		end
...
-- [END DM]
			...
			--print (action.Type .. " " .. tostring(action.OrderPriority));
			if action.Type == "MISSION_ESTABLISH_TRADE_ROUTE" then
				
				local potentialTradeSpots = pActivePlayer:GetUnitAvailableTradeRoutes(unit);
				for i,v in ipairs(potentialTradeSpots) do
					if (v.OldTradeRoute) then
						
						-- Compare and check for auto-renew
						local from = unit:GetPlot():GetPlotIndex();
						local to = Map.GetPlot( v.DestX, v.DestY ):GetPlotIndex();
						for l,m in ipairs(autoRenewRoutes) do
							if --[[m.Type == "TARGETTED" and]] m.From == from and m.To == to and m.Enabled then
								Game.SelectionListGameNetMessage(GameMessageTypes.GAMEMESSAGE_PUSH_MISSION, MissionTypes.MISSION_ESTABLISH_TRADE_ROUTE, to, v.TradeConnectionType, 0, false, nil);
								return;
							end
						end
						...
						instance.UnitActionButton:SetAlpha( 1 );
						--instance.UnitActionIcon:SetColor( 0xFFFFFFFF );  
						instance.UnitActionButton:SetDisabled( false );
						IconHookup(43, actionIconSize, "UNIT_ACTION_ATLAS", instance.UnitActionIcon);
						
						instance.UnitActionButton:RegisterCallback( Mouse.eLClick, function () OnReRouteActionClicked(i); end );
						instance.UnitActionButton:SetVoid1( i );
						instance.UnitActionButton:SetToolTipCallback( ReRouteTipHandler );
						instance.UnitActionButton:RegisterCallback(Mouse.eMouseEnter, function () ShowReTradeRoute(i); end);
						instance.UnitActionButton:RegisterCallback(Mouse.eMouseExit, function () HideReTradeRoute(i); end);
						
						g_prevTradeRoutes[i] = v;
						if g_firstPrevRoute == -1 then g_firstPrevRoute = i; end
					end
				end
			end
		end
...
----------------------------------------------------------------
-- re-trade route functions
----------------------------------------------------------------
function ReRouteTipHandler (control)
	--local unit = UI.GetHeadSelectedUnit();
	local v = g_prevTradeRoutes[control:GetVoid1()];
    ...
    -- Tooltip
	tipControlTable.UnitActionHelp:SetText( "[NEWLINE]" .. strOutput );
	--tipControlTable.UnitActionHelp:SetText( "[NEWLINE]" .. v.DestSiteName .. "[NEWLINE][NEWLINE]" .. strOutput );
	
	-- Title
    tipControlTable.UnitActionText:SetText( "[COLOR_POSITIVE_TEXT]" .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_PREV_ROUTE") .. "[ENDCOLOR]"  .. "  -  " .. v.DestSiteName);
    --tipControlTable.UnitActionText:SetText( "[COLOR_POSITIVE_TEXT]" .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_PREV_ROUTE") .. "[ENDCOLOR]");
    
    -- HotKey
	if g_firstPrevRoute == control:GetVoid1() then
		tipControlTable.UnitActionHotKey:SetText( "(R)" );
	else
		tipControlTable.UnitActionHotKey:SetText( "" );
	end
...
function ShowReTradeRoute ( routeN )
	UI.SetTradeRouteVisibleMode( true );
	local unit = UI.GetHeadSelectedUnit();
	local route = g_prevTradeRoutes[routeN];
	if route == nil then return end
	Game.SelectedUnit_SpeculativePopupTradeRoute_Display(route.DestX, route.DestY, route.TradeConnectionType, unit:GetDomainType());
end

function HideReTradeRoute ( routeN )
	UI.SetTradeRouteVisibleMode( false );
	local route = g_prevTradeRoutes[routeN];
	if route == nil then return end
	Game.SelectedUnit_SpeculativePopupTradeRoute_Hide(route.DestX, route.DestY, route.TradeConnectionType);
end

function OnReRouteActionClicked ( routeN )
	local route = g_prevTradeRoutes[routeN];
	local kPlot = Map.GetPlot( route.DestX, route.DestY );
	Game.SelectionListGameNetMessage(GameMessageTypes.GAMEMESSAGE_PUSH_MISSION, MissionTypes.MISSION_ESTABLISH_TRADE_ROUTE, kPlot:GetPlotIndex(), route.TradeConnectionType, 0, false, nil);
...
function InputHandler( uiMsg, wParam, lParam )
    if(uiMsg == KeyEvents.KeyDown) then
		if (wParam == 18) and (g_firstPrevRoute ~= -1) then
			OnReRouteActionClicked( g_firstPrevRoute );
		end
    end
end
...
function OnUpdateRouteRenewal (Info)
	for l=#autoRenewRoutes-1, 0, -1 do
		local m = autoRenewRoutes[l];
		if m ~= nil then
			if m.From == Info.From and m.To == Info.To then
				table.remove ( autoRenewRoutes, l );
			end
		end
	end
	
	table.insert ( autoRenewRoutes, Info );
end
LuaEvents.UpdateRouteRenewal.Add( OnUpdateRouteRenewal );
