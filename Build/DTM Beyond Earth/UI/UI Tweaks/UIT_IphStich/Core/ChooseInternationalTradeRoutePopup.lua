...
function Initialize()
	...
	local ART_BUFFER_SCROLLAREA			= 211 + 22;
...
g_SortOptions = {
	...
	{"[ICON_ARROW_LEFT] [ICON_RESEARCH] Science", SortByMaxScience},
	--{"[ICON_RESEARCH] Science Advantage", SortByScienceDelta},
	{"[ICON_ARROW_LEFT] [ICON_ENERGY] Energy", SortByMaxEnergy},
	--{"[ICON_ENERGY] Energy Advantage", SortByEnergyDelta},
	{"[ICON_ARROW_LEFT] [ICON_FOOD] Food", SortByFoodInc},
	{"[ICON_FOOD] Food [ICON_ARROW_RIGHT]", SortByFoodOut},
	{"[ICON_ARROW_LEFT] [ICON_FOOD] Food  [ICON_ARROW_RIGHT]", SortByFoodTot},
	{"[ICON_ARROW_LEFT] [ICON_PRODUCTION] Production", SortByProductionInc},
	{"[ICON_PRODUCTION] Production [ICON_ARROW_RIGHT]", SortByProductionOut},
	{"[ICON_ARROW_LEFT] [ICON_PRODUCTION] Production [ICON_ARROW_RIGHT]", SortByProductionTot},
}
g_CurrentSortOption = 1;

function SortData()
	
	local SortFunction = g_SortOptions[g_CurrentSortOption][2]
	
	-- Collapse and expand based on selected sort function
	if (SortFunction == SortByMaxScience or
		SortFunction == SortByMaxEnergy)
	then
		Controls.YourCitiesStack.Collapse();
		Controls.YourOutpostsStack.Collapse();
		Controls.OtherCivsStack.Expand();
		Controls.StationsStack.Expand();
	elseif
	   (SortFunction == SortByFoodInc or
		SortFunction == SortByFoodOut or
		SortFunction == SortByFoodTot or
		SortFunction == SortByProductionInc or
		SortFunction == SortByProductionOut or
		SortFunction == SortByProductionTot)
	then
		Controls.YourCitiesStack.Expand();
		Controls.YourOutpostsStack.Collapse();
		Controls.OtherCivsStack.Collapse();
		Controls.StationsStack.Expand();
	end
	OnCollapseExpand();
	
	table.sort(g_Model, SortFunction);
end
...
function OnConfirmYes( )
	...
	local AutoRenew = {
		--Type = "Targetted",
		From = g_Unit:GetPlot():GetPlotIndex();
		To = kPlot:GetPlotIndex(),
		Enabled = Controls.AutoRenew:IsChecked(),
		FromCity = g_Unit:GetPlot():GetPlotCity(),
		ToCity = kPlot:GetPlotCity()
	}
	if (AutoRenew.ToCity == nil) then AutoRenew.ToCity = kPlot:GetPlotStation() end;
	LuaEvents.UpdateRouteRenewal(AutoRenew);
end
