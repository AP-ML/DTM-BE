...
function RequestYieldDisplay()
	...
	if( bDisplayCivilianYields and
		( (UI.CanSelectionListWork() and bShowYields) or
		  (UI.IsInOrbitalView() and unit:IsOrbitalUnit() and not unit:IsInOrbit())
		) )
	then
...
Events.SerialEventSetOrbitalView.Add(RequestYieldDisplay);
...
function ModOnActivePlayerTurnEnd ()
	-- Exit from orbital view
	if UI.IsInOrbitalView() then
		Events.SerialEventSetOrbitalView( false );
	end
end
Events.ActivePlayerTurnEnd.Add( ModOnActivePlayerTurnEnd );

---------------------------------------------------------------------------------------

local m_IsDisplayPredictedWorkRadius = false;

function CheckColony( p, u, i, j, k, isSelected )
	Events.ClearHexHighlightStyle("OrbitalEffectValidLaunch");
	Events.ClearHexHighlightStyle("OrbitalEffectInvalidLaunch");
	if (isSelected) then
		local pUnit = UI.GetHeadSelectedUnit();
		m_IsDisplayPredictedWorkRadius = false;
		if (pUnit ~= nil) then m_IsDisplayPredictedWorkRadius = pUnit:IsFound(); end
	else
		m_IsDisplayPredictedWorkRadius = false;
	end
end
Events.UnitSelectionChanged.Add( CheckColony );

function OnMouseOverHexColony( hexX, hexY )
	
	if not m_IsDisplayPredictedWorkRadius then return end
	
	Events.ClearHexHighlightStyle("OrbitalEffectValidLaunch");
	Events.ClearHexHighlightStyle("OrbitalEffectInvalidLaunch");
	
	local CentralPlot = Map.GetPlot(hexX, hexY);
	local MaxDistance = GameDefines.MAXIMUM_BUY_PLOT_DISTANCE;
--MAX_CITY_DIST_HIGHWATER_MARK
--MIN_CITY_RANGE
	local HighlightStyle = "OrbitalEffectValidLaunch"
	
	if not Players[Game.GetActivePlayer()]:CanFound(hexX, hexY) then HighlightStyle = "OrbitalEffectInvalidLaunch"; end
	
	for x=hexX-4, hexX+4, 1 do
		for y=hexY-4, hexY+4, 1 do
			if Map.PlotDistance(hexX, hexY, x, y) <= MaxDistance then
				local hexPos = ToHexFromGrid( Vector2( x, y ) );
				Events.SerialEventHexHighlight(hexPos, true, Vector4(), HighlightStyle );
			end
		end
	end
end
Events.SerialEventMouseOverHex.Add( OnMouseOverHexColony );

---------------------------------------------------------------------------------------
-- MiasmaNotification
-- Author: Vicevirtuoso
-- DateCreated: 11/5/2014 2:17:07 PM
--------------------------------------------------------------


local iBaseMiasmaDamage = GameDefines.MIASMA_HEALTH_MOD_BASE
local iMaxCivs = GameDefines.MAX_MAJOR_CIVS


--Cache all player perks, techs, promotions which affect Miasma Damage
local tTechnologies = {}

for row in GameInfo.Technologies() do
	if row.AllowsMiasmaImmunity == true or row.UnitBaseMiasmaHeal > 0 then
		tTechnologies[row.ID] = true
	end
end
...
for row in GameInfo.PlayerPerks() do
	if row.MiasmaDamageMod > 0 then
		if not tPerks[row.ID] then tPerks[row.ID] = {} end
		tPerks[row.ID].MiasmaDamageMod = row.MiasmaDamageMod
	end
	if row.MiasmaBaseHeal > 0 then
		if not tPerks[row.ID] then tPerks[row.ID] = {} end
		tPerks[row.ID].MiasmaBaseHeal = row.MiasmaBaseHeal
	end
end
...
for row in GameInfo.UnitPromotions() do
	if row.MiasmaImmuneWorkers == true then
		tPromotions[row.ID] = true
	end
end
...
function OnPlayerDoTurn(iPlayer)
	if iPlayer < iMaxCivs then
		...
		--no reason to run this for AIs
		if not pPlayer:IsHuman() then return false end

		local iDamage = iBaseMiasmaDamage

		local pTeamTechs = Teams[pPlayer:GetTeam()]:GetTeamTechs()
		for k, v in pairs(tTechnologies) do
			if pTeamTechs:HasTech(k) then
				--Since all tech-based tags nullify damage taken, the function will end if the player has one
				return false
			end
		end

		for k, v in pairs(tPerks) do
			if pPlayer:HasPerk(k) then
				if v.MiasmaDamageMod then
					iDamage = iDamage - v.MiasmaDamageMod
				elseif v.MiasmaBaseHeal then
					--this means that the player's units heal in Miasma and thus this function isn't needed, so return
					return false
				end
			end
		end
		...
		for pUnit in pPlayer:Units() do
			if pUnit:GetPlot():HasMiasma() then
				local bHasPromotion = false;
				...
				if bHasPromotion ~= true then
					...
					if iCurHP <= (-iDamage) then
						local sText = "Your " .. pUnit:GetName() .. " will succumb to the Miasma next turn if you do not move it to safety!"
						--Locale.ConvertTextKey("TXT_KEY_VV_NOTIFICATION_MIASMA_DEATH_IMMINENT_TEXT", pUnit:GetName())
						local sTitle = "Unit Nearing Death by Miasma!"
						--Locale.ConvertTextKey("TXT_KEY_VV_NOTIFICATION_MIASMA_DEATH_IMMINENT_TITLE")
						pPlayer:AddNotification(NotificationTypes.NOTIFICATION_KILLED_COVERT_AGENT, sText, sTitle, pUnit:GetX(), pUnit:GetY())
					end
				end
			end
		end
