...
function DisplayData()
	...
	if (g_Data ~= nil) then
		for i,v in ipairs(g_Data) do
			...
			local Void1 = -1 * i;
			instance.AutoRenew:SetCheck(false);
			instance.AutoRenew:SetHide(not g_CurrentTab == "YourTR");
			for l,m in ipairs(autoRenewRoutes) do
				if m.FromCity:GetName() == v.OriginSiteName and m.ToCity:GetName() == v.DestSiteName then
					instance.AutoRenew:SetCheck(m.Enabled);
					Void1 = l
					break;
				end
			end
			instance.AutoRenew:RegisterCheckHandler(function (b) OnCheckAutoRenew(Void1, b) end );
...
function OnCheckAutoRenew (Void1, bIsChecked)
	
	if Void1 > 0 then
		
		local RenewData = autoRenewRoutes[Void1];
		RenewData.Enabled = bIsChecked;
		
	elseif Void1 < 0 then
		
		local Route = g_Data[Void1 * -1];
		local RenewData = {--[[Type = "TARGETTED",]] Enabled = bIsChecked};
		
		local FromName = Route.OriginSiteName;
		local ToName = Route.DestSiteName;
		
		-- search for 'from' city from the active player's city list
		for pCity in Players[ Game.GetActivePlayer() ]:Cities() do
			if pCity:GetName() == FromName then
				RenewData.From = pCity:Plot():GetPlotIndex();
				RenewData.FromCity = pCity;
				break;
			end
		end
		
		-- search ALL players for the to city
		for _,pPlayer in ipairs(Players) do
			for pCity in pPlayer:Cities() do
				if pCity:GetName() == ToName then
					RenewData.To = pCity:Plot():GetPlotIndex();
					RenewData.ToCity = pCity;
					break;
				end
			end
		end
		
		-- if none found, search the stations
		if (RenewData.ToCity == nil) then
			for stationIndex = 0, Game.GetNumActiveStations() - 1, 1 
			do
				local station = Game.GetStationByIndex( stationIndex );
				if (station:GetName() == ToName) then
					RenewData.To = Map.GetPlot(station:GetX(), station:GetY()):GetPlotIndex();
					RenewData.ToCity = station;
					break;
				end
			end
		end
		...
		print(RenewData.To)
		print(RenewData.ToCity)
		print(RenewData.From)
		print(RenewData.FromCity)
		print(RenewData.Enabled)
		
		LuaEvents.UpdateRouteRenewal(RenewData);
