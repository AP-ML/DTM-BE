...
local BaseOffset = Vector2 ( 0, 20 );
--local GarrisonOffset = Vector2( -43, -39 ); -- default, topleft
local GarrisonOffset = Vector2( -48, -44 ); -- fixed topleft
local GarrisonOtherOffset = Vector2( -55, -34 );
local CityNonGarrisonOffset = Vector2( 48, -44 );
local CivilianOffset = Vector2( 45, -35 );
local InOrbitOffset = Vector2( 0, -50 );
local InOrbitCityOffset = Vector2( 0, -70 );
local UnlaunchedOrbital = Vector2 ( 45, 45 );
local CarrierOffset = Vector2 (-25, 20);
...
--                self.m_Instance.Anchor:ChangeParent( Controls.AirCraftFlags );
    ...
    UpdateDimmedState = function( self )
		...
		local pUnit = Players[ self.m_PlayerID ]:GetUnitByID( self.m_UnitID );
		if (pUnit ~= nil) then
			local activity = pUnit:GetActivityType();
			-- force dim for automated trade units
			if self.m_IsTrade and pUnit:IsAutomated() then forceDim = true; end
			-- force dim on sleeping aircraft
			if self.m_IsAirCraft and activity == ActivityTypes.ACTIVITY_SLEEP then forceDim = true; end
			-- force dim for non-active orbital units when viewing surface
			if pUnit:IsInOrbit() and not UI.IsInOrbitalView() and activity == ActivityTypes.ACTIVITY_SLEEP then forceDim = true; end
			-- force dim for non-orbatal units when orbital view
			if not pUnit:IsInOrbit() and UI.IsInOrbitalView() then forceDim = true; end
		end
		
		if( forceDim or (self.m_IsDimmed and not self.m_OverrideDimmedFlag) ) then
	...
	UpdateFlagOffset = function( self )
        ...
        local offset = Vector2(BaseOffset.x, BaseOffset.y);
		
		-- Orbital units
		if (pUnit:IsInOrbit()) then
			if (plot:IsCity()) then
				offset = InOrbitCityOffset;
			else
				offset = InOrbitOffset;
			end
			
		-- Carrier and Cargo Units
		elseif pUnit:CargoSpace() > 0 or pUnit:IsCargo() then
			local DISTANCE_BETWEEN_ICONS = 50;
			...
			for i=0, numUnits-1 do
				local sUnit = plot:GetUnit(i);
				if (sUnit:IsCargo()) then
					if sUnit == pUnit then thisFlag = numCargo; end
					numCargo = numCargo + 1;
				end
			end
			
			offset.x = DISTANCE_BETWEEN_ICONS * (0.5 - numCargo / 2.0 + thisFlag);
			
		-- Aircraft units at a city
		elseif self.m_IsAirCraft then
			local HEIGHT_OF_ICONS = 40;
			local DISTANCE_BETWEEN_ICONS = 50;
			
			local numUnits = plot:GetNumUnits();
			local thisAircraft = 0;
			local numAircraft = 0;
			
			for i=0, numUnits-1 do
				local sUnit = plot:GetUnit(i);
				if (sUnit:GetDomainType() == DomainTypes.DOMAIN_AIR) then
					if sUnit == pUnit then thisAircraft = numAircraft; end
			...
			if city:CanRangeStrikeNow()
				or (not city:IsCapital() and player:IsCapitalConnectedToCity(city) and not city:IsBlockaded())
				then
				if thisAircraft >= numAircraft / 2 then thisAircraft = thisAircraft + 2; end
				numAircraft = numAircraft + 2;
			end
			
			offset.y = HEIGHT_OF_ICONS;
			offset.x = DISTANCE_BETWEEN_ICONS * (0.5 - numAircraft / 2.0 + thisAircraft);
