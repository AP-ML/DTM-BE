...
--local WorldPositionOffset2					= WorldPositionOffset;
...
local m_isImprovementProjectsHeadingOpen= false;
local m_isWonderHeadingOpen				= false;
local m_isBuildingHeadingOpen			= false;
local m_isSpecialistsHeadingOpen		= false;
local g_isCityContentsListOpen			= true;
...
local purchasePlotButtons = {}
local m_isClassicView = false;
local m_isAutoCollapse = false;
...
function CityScreenClosed()
	...
	-- Auto collapse building categories
	if m_isAutoCollapse then
		m_isImprovementProjectsHeadingOpen 	= false;
		m_isWonderHeadingOpen 				= false;
		m_isBuildingHeadingOpen 			= false;
		m_isSpecialistsHeadingOpen 			= false;
	end
...
function AddBuildingButton( city, building )
	...
	if (city:IsHasBuilding(buildingID)) then
		...
		if building.SpecialistType and strBuildingStats == "" then strBuildingStats = " "; end
		...
		local minSize = controlTable.BuildingImage:GetSizeY() + 6;
		if totalHeight < minSize then totalHeight = minSize; end
...
--	Controls.EndTurnText:SetText(Locale.ConvertTextKey("TXT_KEY_CITYVIEW_RETURN_TO_MAP"));
	...
	if g_isProductionQueueOpen and (qLength >= MAX_QUEUE_ITEMS) then
	...
	if qLength == 1 then
		...
		Controls.b1QueueText:SetOffsetX (10);
	else
		Controls.b1remove:SetHide( false );
		Controls.b1QueueText:SetOffsetX (48);
	end
...
function UpdateHexHighlights()
	...
	if (UI.IsCityScreenUp()) then   
		...
		if  uiMode == InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT then
			if not m_isClassicView then
				for i = 0, city:GetNumCityPlots() - 1, 1 do
					local plot = city:GetCityIndexPlot( i );
					
					if (plot ~= nil) then
						local hexPos = ToHexFromGrid( Vector2( plot:GetX(), plot:GetY() ) );
						local highlightColor = Vector4( 1, 1, 1, 0 );
						if (i == 0) then  highlightColor = Vector4( 0.9, 0.9, 0.9, 1 );
						elseif (city:CanBuyPlotAt(plot:GetX(), plot:GetY(), true)) then highlightColor = Vector4( 0.95, 0.8, 0.2, 0 );
						elseif (city:IsWorkingPlot( plot ) and city:IsForcedWorkingPlot( plot )) then highlightColor = Vector4( 0.9, 0.9, 0.9, 1 );
						elseif (city:IsWorkingPlot( plot )) then highlightColor = Vector4( 0.2, 0.85, 0.3, 1 );
						elseif (city:CanWork( plot )) then highlightColor = Vector4 ( 0.95, 0.55, 0.25, 0 ); --Vector4 ( 0.35, 0.35, 1, 2 );
						end
						Events.SerialEventHexHighlight(hexPos, true, highlightColor, genericUnitHexBorder );
						Events.SerialEventHexHighlight(hexPos, true, Vector4(), "OrbitalEffectInvalidLaunch" );
					end
				end
			end
...
function UpdateWorkingHexes()
	...
	if (UI.IsCityScreenUp()) then   
		...
		-- Buy plot buttons		
		local uiMode = UI.GetInterfaceMode();
		local worldOffset = WorldPositionOffset;
		if (m_isClassicView) then worldOffset = WorldPositionOffset2; end
			...
			for i = 0, city:GetNumCityPlots() - 1, 1 do
				...
				if (plot ~= nil) then
					...
					if (city:CanBuyPlotAt(plot:GetX(), plot:GetY(), false)) then						
						...
						controlTable.BuyPlotButtonAnchor:SetWorldPosition( VecAdd( worldPos, worldOffset ) );						
					...
					if not m_isClassicView and not UI.IsTouchScreenEnabled() then
						controlTable.BuyPlotAnchoredButton:SetHide( true );
						purchasePlotButtons[i] = controlTable;
					end
				end
			end
		end
...
function OnMouseOverHex( hexX, hexY )
	...
	if not m_isClassicView and not UI.IsTouchScreenEnabled() then
		for i,j in pairs(purchasePlotButtons) do
			local plot = city:GetCityIndexPlot( i );
			local hide = true;
			if city:CanBuyPlotAt(hexX, hexY, true) and (hexX == plot:GetX() and hexY == plot:GetY()) then
				hide = false
			end
			j.BuyPlotAnchoredButton:SetHide( hide );
		end
	end
end
...
-- ===========================================================================
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT][MouseEvents.LButtonUp] = 
function( wParam, lParam )
	if m_isClassicView then return; end
...
function OnClickPlot ( plot )
	local city = UI.GetHeadSelectedCity();
	
	for i = 0, city:GetNumCityPlots() - 1, 1 do
		if (city:GetCityIndexPlot( i ) == plot) then
			
			if (i == 0) then -- clicking on city itself
				OnResetForcedTiles();
			elseif (city:CanBuyPlotAt(plot:GetX(), plot:GetY())) then
				OnConfirmBuyPlotButtonClicked( i, purchasePlotButtons[i].BuyPlotAnchoredButton:GetVoid2() );
			else
				PlotButtonClicked (i);
			end
		end
...
-- ===========================================================================
function OnToggleClassicView( bIsChecked )
	m_isClassicView = not bIsChecked;
	UpdateWorkingHexes();
end
Controls.ClassicWorkersButton:RegisterCheckHandler( OnToggleClassicView );

function OnToggleAutoCollapse (bIsChecked)
	m_isAutoCollapse = bIsChecked;
end
Controls.ToggleAutoCollapse:RegisterCheckHandler( OnToggleAutoCollapse );
