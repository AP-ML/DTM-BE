...
-- CHANGES
			-- local yieldTooltip = v[2];
			-- go through improvement-bonus-to-icon XML file;
			-- 
-- END CHANGES
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
			thisButton:SetHide( false );
...
function AdjustArtOnGrantedUnitButton( thisButton, thisUnitInfo, textureSize )
	...
	if thisButton then
		...
		local colorTextureSheet = textureSheet;
		if textureOffset ~= nil then
			if textureSize == 45 then
				colorTextureSheet = "colorweb_"..colorTextureSheet;	
			end
		end			
		if textureOffset == nil then
			colorTextureSheet = defaultErrorTextureSheet;
			textureOffset = nullOffset;
		end				
		thisButton:SetTexture( colorTextureSheet );
